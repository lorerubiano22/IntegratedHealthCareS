...
public class Route {
	...
	public void computeTravelTime(Inputs inp) {
		...
		for(int i=1;i<list.size();i++) {
			SubJobs origin=list.get(i-1);
			SubJobs end=list.get(i);
			double tv=inp.getCarCost().getCost(origin.getId()-1, end.getId()-1);
			travelTimeDuration+=tv;
		}
	...
	public void checkingDetour(Test test, Inputs inp, Solution initialSol) {
		...
		for(Edge e: this.edges.values()) {
			...
			if(origen.getSubJobKey().equals("D2") && end.getSubJobKey().equals("P56")) {
				System.out.println(" Edge "+ e.toString());
			}
	...
	public void settingConnections(Solution solution, Test test, Inputs inp) {
		...
		for(SubJobs j: this.subJobsList) {
			...
			if(originNode.getSubJobKey().equals("D55")) {
				System.out.println("Sol");	
			}
		}
	...
	public void settingEdgesTimesRoute(Solution solution, Test test, Inputs inp) {
		// setting las connexiones pick-drop 
		for(Edge e: this.edges.values()) {
			double distanceInRoute=computingDistance(e,inp,solution,test);
			if(distanceInRoute>10) {
