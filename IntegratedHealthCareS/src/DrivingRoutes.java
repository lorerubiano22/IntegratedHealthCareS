...
public class DrivingRoutes {
	...
	private Solution solutionGenerator(ArrayList<Jobs> insertionOrder, ArrayList<Parts> poolParts, HashMap<String, Couple> dropoffHomeCareStaff,
		...
		do {
			for(Jobs j: insertionOrder) {
				if(j.getSubJobKey().equals("P23")) {
					System.out.println("Stop");
				}
				if(j.getSubJobKey().equals("D23")) {
					System.out.println("Stop");
				}
				boolean inserted= insertionJobSelectingRoute(poolParts,j,dropoffHomeCareStaff,dropoffpatientMedicalCentre,pickpatientMedicalCentre,pickupHomeCareStaff,toInsert);
				if(inserted) {
			...
			for(ArrayList<Parts> highQualification: shifthomeCareStaffQ2) { // jobs qualification 1 -> home care staff cualification 2
				if(!lowQualification.isEmpty()) {
					for(Parts p: lowQualification) { // intentando de insertar parte por parte
						...
						if(lastJobLow.getDepartureTime()<=firstJobHigh.getArrivalTime()) { // insert part early  part(low)--- part(high-high)
							if(Math.abs(firstJobLow.getArrivalTime()-lastJobHigh.getDepartureTime())<=test.getWorkingTime()) { // working hours
								highQualification.add(0,p);
							}
						}
						else {
							if(lastJobHigh.getDepartureTime()<=firstJobLow.getArrivalTime()) { // insert part early  part(low)--- part(high-high)
								if(Math.abs(firstJobHigh.getArrivalTime()-lastJobLow.getDepartureTime())<=test.getWorkingTime()) { // working hours
	...
	private boolean assigningJobsToMedicalStaff(SubJobs j, ArrayList<Parts> shift, HashMap<String, Couple> dropoffHomeCareStaff2, HashMap<String, Couple> pickupHomeCareStaff2, HashMap<String, Couple> dropoffpatientMedicalCentre2, HashMap<String, Couple> pickpatientMedicalCentre2) {
		...
		if(shift.isEmpty()) {
			...
			if(shift.get(shift.size()-1).getListSubJobs().get(shift.get(shift.size()-1).getListSubJobs().size()-1).getDepartureTime()<p.getListSubJobs().get(0).getArrivalTime()) {
				double start =shift.get(0).getListSubJobs().get(0).getArrivalTime();
				double end= p.getListSubJobs().get(p.getListSubJobs().size()-1).getDepartureTime();
				if(p.getListSubJobs().get(p.getListSubJobs().size()-1).getDepartureTime()-shift.get(0).getListSubJobs().get(0).getArrivalTime()<test.getWorkingTime()) {
				inserted=true;
				shift.add(p);
			}
			}	
	...
	private boolean validationSequenceRoutes(ArrayList<Route> poolRoutes) {	
		...
		if(wrongDepartureArrivalTime.isEmpty() && wrongHorasInicioServicio.isEmpty()) {
			feasible= true;
		}
	...
	private boolean insertionJobSelectingRoute(ArrayList<Parts> poolParts, Jobs j, HashMap<String, Couple> dropoffHomeCareStaff2, HashMap<String, Couple> dropoffpatientMedicalCentre2, HashMap<String, Couple> pickpatientMedicalCentre2, HashMap<String, Couple> pickupHomeCareStaff, HashMap<String, SubJobs> toInsert) {
		...
		for(Parts p: bestRoutes) {
			...
			if(j.getSubJobKey().equals("D21")) {
				System.out.println("Stop");
			}			if(p.getDirectorySubjobs().containsKey("P28") && j.getSubJobKey().equals("D21")) {
				System.out.println("Stop");
			}
			if(p.getDirectorySubjobs().containsKey("P28") && j.getSubJobKey().equals("P65")) {
				System.out.println("Stop");
			}

			if(p.getDirectorySubjobs().containsKey("P4965") && j.getSubJobKey().equals("P39")) {
				System.out.println("Stop");
			}
	...
	private boolean inseringRoute(Jobs j, Parts p, HashMap<String, Couple> dropoffHomeCareStaff2, HashMap<String, Couple> dropoffpatientMedicalCentre2, HashMap<String, Couple> pickpatientMedicalCentre2, HashMap<String, Couple> pickupHomeCareStaff2) {
		...
		if(p.getListSubJobs().isEmpty() ) {
			...
			double s=0;
			ArrayList<SubJobs> sequence= insertingJob(p,subJobsList,dropoffpatientMedicalCentre2,dropoffHomeCareStaff2,pickpatientMedicalCentre2,pickupHomeCareStaff2);
			//			ArrayList<SubJobs> sequence= new ArrayList<SubJobs>();
			//			for(SubJobs jobsInRoute:p.getListSubJobs()) {
			//				sequence.add(jobsInRoute);
			//			}
			//			for(SubJobs jobsInRoute:subJobsList) {
			//				sequence.add(jobsInRoute);
			//			}
			//			sequence.sort(Jobs.SORT_BY_STARTW);
			if(!sequence.isEmpty()) {
	...
	private boolean waintingTime(ArrayList<SubJobs> sequence) {
		// evalua los tiempos de espera
		boolean feasible=false;
		ArrayList<SubJobs> copy=new ArrayList<SubJobs>();
		for(SubJobs j:sequence) {
			copy.add(new SubJobs(j));
		}

		SubJobs last=copy.get(copy.size()-1);
		// Arrival
		double aux=determineAdditionalTime(last);
		double arrivalTime=last.getstartServiceTime()-aux;
		updatingTime(last,copy);
		for(int j=copy.size()-1;j>0;j--) {
			SubJobs b=copy.get(j);
			SubJobs a=copy.get(j-1);
			// calculo del departure en a para respetar 
			double tv=inp.getCarCost().getCost(a.getId()-1, b.getId()-1);
			double departure=arrivalTime-tv;
			double additionalTimeLeaving=computingLeavlingNode(a);
			arrivalTime=departure-additionalTimeLeaving;
			double possibleStartTime=0;
			double calculatedStartTime=0;
			if(a.isMedicalCentre() ) {
				if(a.getTotalPeople()<0) {
					calculatedStartTime=(arrivalTime+test.getRegistrationTime());
					possibleStartTime=Math.max(a.getEndTime(), calculatedStartTime);
				}
				else {
					calculatedStartTime=(arrivalTime);
					possibleStartTime=Math.max(a.getStartTime(), calculatedStartTime);
				}
			}
			else {
				if(a.isClient()) {
					calculatedStartTime=(arrivalTime+test.getloadTimeHomeCareStaff());
				}
				else {
					calculatedStartTime=(arrivalTime+test.getloadTimePatient());
				}
				possibleStartTime=Math.max(a.getSoftStartTime(), calculatedStartTime);
			}

			if(possibleStartTime>=a.getSoftStartTime() && possibleStartTime<=a.getSoftEndTime() ) {
				continue;
			}
			else {
				break;
	...
	private void updatingTimes(ArrayList<SubJobs> sequence) {
		// timeMatrix - patient = 1, -1, 1,-1
		// timeMatrix - client = -1, 1
		for(SubJobs j: sequence) {
			if(j.getTotalPeople()<0) {
				if(j.isClient()) {
					timeMatrix[j.getId()-1][0]=j.getstartServiceTime();
				}
				else {
					if(j.isPatient()) {
						timeMatrix[j.getId()-1][3]=j.getstartServiceTime();
					}
					else {// medical centre
						timeMatrix[j.getIdUser()-1][1]=j.getstartServiceTime();
					}
				}
				...
				if(j.isClient()) {
					timeMatrix[j.getId()-1][1]=j.getstartServiceTime();
				}
				else {
					if(j.isPatient()) {
						timeMatrix[j.getId()-1][0]=j.getstartServiceTime();
						...
						timeMatrix[j.getIdUser()-1][2]=j.getstartServiceTime();
					}
				}
			}
		}
		...
		for(SubJobs j: sequence) {
			...
			if(j.isPatient()) {
				if(j.getTotalPeople()>0) {
					double travelTime=inp.getCarCost().getCost(j.getId()-1, j.getsubJobPair().getId()-1);
					double endTime=timeMatrix[j.getId()-1][1]-(test.getloadTimePatient()+test.getRegistrationTime())-travelTime;
					j.setEndTime(endTime);
					j.setStartTime(endTime-test.getCumulativeWaitingTime());
					double start=endTime-test.getCumulativeWaitingTime();
				}
	...
	private void checkingWaitingTimes(Parts p) {
		//		double  [] waitingTimes=new double[p.getListSubJobs().size()];
		//		int i=-1;
		//		for(SubJobs j:p.getListSubJobs()) {
		//			i++;
		//			double additionalTime=determineAdditionalTime(j);
		//			waitingTimes[i]=j.getstartServiceTime()-(j.getArrivalTime()+additionalTime);
		//			}
		//		for(int j=0;j<waitingTimes.length;j++) {
		//			System.out.println(p.getListSubJobs().get(j).getSubJobKey()+"_"+waitingTimes[j]);
		//		}
		//		double max=maxValue(waitingTimes);
		ArrayList<SubJobs> copy=new ArrayList<SubJobs>();
		for(SubJobs j:p.getListSubJobs()) {
			copy.add(new SubJobs(j));
		}
		...
		if(vehicleCapacityPart(sequence)) {
			...
			for(int index=1;index<sequence.size();index++) {
				...
				if(startServiceTime-(arrivalTime+additionalTime)<=test.getCumulativeWaitingTime()) {
					...
					if(b.isClient() ) {
						//if(b.getTotalPeople()<0) {
						if(arrivalTime>=(b.getSoftStartTime()) && startServiceTime<=b.getSoftEndTime()) {
							feasible=true;
						}
						if(!feasible) {
							break;	
	...
	private ArrayList<SubJobs> insertingJob(Parts p, ArrayList<SubJobs> subJobsList2, HashMap<String, Couple> dropoffpatientMedicalCentre2, HashMap<String, Couple> dropoffHomeCareStaff2, HashMap<String, Couple> pickpatientMedicalCentre2, HashMap<String, Couple> pickupHomeCareStaff2) {
		ArrayList<SubJobs> hardConstraintsJobs = jobsHardTimeWindow(p,subJobsList2); // selection of jobs con hard time window

		ArrayList<SubJobs> list = integratingJobs(hardConstraintsJobs); // se insertan

		ArrayList<SubJobs> estimatedTimes= jobsEstimatedTimeWindow(subJobsList2,p,dropoffpatientMedicalCentre2,dropoffHomeCareStaff2,pickpatientMedicalCentre2,pickupHomeCareStaff2);

		estimatedTimes.sort(Jobs.SORT_BY_STARTW);

		//insertingJobsEstimatedTimeWindow(estimatedTimes,list);
	...
	private ArrayList<SubJobs> integratingJobs(ArrayList<SubJobs> hardConstraintsJobs) {
		...
		//	boolean inserted=checkingSequenceVehicleCapacity(list);
		//boolean inserted=testing(list);
	...
	private boolean checkingSequenceVehicleCapacity(ArrayList<SubJobs> sequence) {
		// Analizar detour desde el depot
		boolean inserted=false;
		if(!sequence.isEmpty()) {	
			double departure_a=sequence.get(0).getDepartureTime();
			if(vehicleCapacityPart(sequence)) {
				//	if(vehicleCapacityPart(sequence) && depotDetour(sequence)) {
				inserted=true;
				for(int index=1;index<sequence.size();index++) {
					SubJobs a=sequence.get(index-1);
					SubJobs b=sequence.get(index);
...
//			}
	...
	private void slackTime(ArrayList<Route> poolRoutes) {
		for(Route r: poolRoutes) { // calculating the start service time
			settingArrivalDepartureTime(r);// setting arrival times and departure time
			settingStartEndServiceTime(r);// setting arrival times and departure time
		}
...
//
//		Solution vecindario3= chain(iteration,vecindario2);
//
//		if(vecindario3.getobjectiveFunction()<=solsorting.getobjectiveFunction()) {
...
//		}
		...
		System.out.println("Solution before inter changes" + vecindario2);
		System.out.println("Solution before inter changes");
...
//		}
	...
	private Solution chain(int iteration, Solution baseSol) {
		// ojo con el detour
		Solution sol=new Solution(baseSol);
		Solution newSol=new Solution();

		boolean goodSolution=false;
		boolean feasibleStaff=false;
		// Selection of part
		...
		for(Route r:sol.getRoutes()) {
			for(Parts part:r.getPartsRoute()) {
				...
				for(SubJobs j:part.getListSubJobs()) {
					if(j.getId()!=1) {
						SubJobs jobs= new SubJobs(j);
						p.getDirectorySubjobs().put(jobs.getSubJobKey(), jobs);
						p.getListSubJobs().add(jobs);
					}
				}
				if(!p.getListSubJobs().isEmpty()) {
					partList.add(p);
		...
		int index1=0;
		int index2=0;
		while(index1==index2) {// seleccionar 2 rutas aleatorias
			index1=rn.nextInt(partList.size());
			index2=rn.nextInt(partList.size());
		}



		Parts toDestroy=shorterRoute(partList);// seleccionar la ruta de longitud más corta
		if(toDestroy!=null) {
			ArrayList<SubJobs> listSubJobs= transportRequest(toDestroy.getListSubJobs());
			ArrayList<Parts> partslistRemaing= remaingParts(partList,listSubJobs);

			newSol= constructionSol(listSubJobs,partslistRemaing,sol);
		...
		if(newSol.getobjectiveFunction()<sol.getobjectiveFunction()) {
			sol= new Solution(newSol);	
		}
		else {
			sol= new Solution(baseSol);
		}		
	...
	private ArrayList<SubJobs> selectionTransportRequest(int iteration, ArrayList<Parts> partList) {
	...
	private ArrayList<SubJobs> seleccionarTransportRequest(Route toDestroy) {
		// seleccionar partes que que no contengan clientes
		ArrayList<Parts> partsRequest= new ArrayList<Parts>();
		for(Parts p:toDestroy.getPartsRoute()) {
		...
		ArrayList<SubJobs> transportRequest= new ArrayList<SubJobs>();
		for(Parts p:partsRequest) {
			for(SubJobs j:p.getListSubJobs()) {
				if(j.isMedicalCentre() || j.isClient()) {
					transportRequest.add(new SubJobs(j));
				}
			}
		}
		int totalRequest=rn.nextInt(transportRequest.size());
		ArrayList<SubJobs> request= new ArrayList<SubJobs>();
		while(request.size()<totalRequest) {
			int index=rn.nextInt(transportRequest.size());
			if(!request.contains(transportRequest.get(index))) {
				request.add(transportRequest.get(index));
			}
		}

		ArrayList<SubJobs> remainingPart= new ArrayList<SubJobs>();
		for(SubJobs j: request) {
			if(j.isMedicalCentre()) {
				String key="";
				if(j.getTotalPeople()<0) { // drop off
					key="P"+j.getIdUser();
				...
				SubJobs part=selectingRemainingJob(key,toDestroy);
				if(part!=null) {
					changingTimeWindow(j,part);
					remainingPart.add(part);	
				}
				else {
					System.out.println("Stop");
				}
	...
	private void changingTimeWindow(SubJobs j, SubJobs part) {
		double tv=0;
		if(j.getTotalPeople()<0) { // drop off MC
			// cambiar la hora del pick up patient home part
			tv=inp.getCarCost().getCost(part.getId()-1, j.getId()-1)*test.getDetour();
			part.setStartTime(j.getStartTime()-tv);
			part.setEndTime(j.getEndTime()-tv);
		}
		else { //pick up MC
			// cambiar la patient homne
			tv=inp.getCarCost().getCost(j.getId()-1,part.getId()-1)*test.getDetour();
			part.setStartTime(j.getStartTime()+tv);
			part.setEndTime(j.getEndTime()+tv);
		}
	...
	private SubJobs selectingRemainingJob(String key, Route toDestroy) {
		SubJobs part=null;
		for(SubJobs j: toDestroy.getSubJobsList()) {
			if(j.getSubJobKey().equals(key)) {
				part=j;
				break;
			}
		}
	...
	private Route otherRoute(ArrayList<Route> routeList, Route toDestroy) {
		Route r=null;
		do {
			int index=rn.nextInt(routeList.size());
			r=routeList.get(index);
		}
		while(toDestroy==r || r==null);


		return r;
	}

	private Parts shorterRoute(ArrayList<Parts> routeList) {
		// la condición de la ruta es que tenga partes con nodos no relevantes
		int index=rn.nextInt(routeList.size());
		Parts route=routeList.get(index);
		return route;
	}
	...
	private Solution movement(int iteration, Solution baseSol) {
		...
		//for(int i=0;i<5;i++) {
			...
			do {
				...
				newSol= new Solution();
				int iter=-1;
				int randomR=rn.nextInt(currentSol.getRoutes().size());// seleccionar una ruta aleatoria
